...
void Scene::change_theme(const string& theme, const string& path) {
...
void Scene::PlayBGM(const string& MusicPath) {
    string op = "open " + MusicPath + " alias BGM";
    mciSendString(op.c_str(), nullptr, 0, nullptr);
    mciSendString("play BGM repeat", nullptr, 0, nullptr);
}

void Scene::PlayMusic(const string& MusicPath) {
    string op = "open " + MusicPath + " alias soundeffect";
    mciSendString("close soundeffect", nullptr, 0, nullptr);
    mciSendString(op.c_str(), nullptr, 0, nullptr);
    mciSendString("play soundeffect", nullptr, 0, nullptr);
}
...
void Scene::updateWithoutInput() {
    ...
    if (player.playerStatus == fly_left1 || player.playerStatus == fly_left2 || player.playerStatus == fly_right1 ||
        player.playerStatus == fly_right2) {
        player.autoJump_fly();
        if (player.vy == 0 && land.land_vy <= GRAVITY && land.land_vy >= -GRAVITY) {
...
void Scene::updateWithKeyInput() {
    if (keymsg()) {
        ...
        if (kmsg.flag == KEY_UP) {
            if ((kmsg.key == 'A' || kmsg.key == 'a' || kmsg.key == 37) ||
                (kmsg.key == 'D' || kmsg.key == 'd' || kmsg.key == 39)) {
                direct = 0;
            }
        }
    }
