...
#define DOODLE_JUMP_WIDTH 500
#define DOODLE_JUMP_HEIGHT 800
#define LAND_NUM 10
#define GRAVITY 0.15
using namespace std;
enum InterfaceType {
...
public:
    ...
    void change_theme(const string& theme, const string& path = "image/Theme/");

    void draw();

    void PlayBGM(const string& MusicPath = "sounds/bgm.mp3");
...
public:
    ...
    Land(const string &path = "image/Land/");
...
public:
    ...
    Player(string path = "image/Theme/", const string& theme = "Classic");

    void retry_clean();

    void change_theme(const string& theme, string path = "image/Theme/");
