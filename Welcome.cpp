...
Welcome::Welcome() {
	...
	setup(_Doodle_Jump_Width_, _Doodle_Jump_Height_, 0, 0, false);
...
void Welcome::rank_save() {
	...
	for (auto i = scorerank_map.begin(); i != scorerank_map.end(); i++)
...
void Welcome::main_interface() {
	p_mouseget = thread(welcome_mouseget, this);
	p_mouseget.detach();
...
void Welcome::show_rank(InterfaceType pre) {
	...
	while (interfacetype == I_rank) {
		msmsg = getmouse();
		if (msmsg.flag == BUTTON_UP)
			if (!(msmsg.x >= -210 && msmsg.x <= 210 && msmsg.y >= -190 && msmsg.y <= 190) || (msmsg.x >= 75 && msmsg.x <= 175 && msmsg.y >= -165 && msmsg.y <= -115))
				interfacetype = pre;
	}
}
	...
	while (interfacetype == I_option) {
		...
		if (msmsg.flag == BUTTON_UP) {
			if (msmsg.x >= -20 && msmsg.x <= 80 && msmsg.y >= 85 && msmsg.y <= 135) {
				if (bgm)mciSendString("close BGM", NULL, 0, NULL);
				else scene.PlayBGM();
				bgm = !bgm;
...
void Welcome::draw_jump() {
	...
	vy += _Gravity_;
	y_bottom += vy;
	moveto(tmp_land.middle_x, -(tmp_land.top_y - _Doodle_Jump_Height_ / 2));
	showimage(tmp_land.im_land);
	moveto(x_middle, -(y_bottom - player.height / 2 - _Doodle_Jump_Height_ / 2));
...
void Welcome::game_run() {
	p_mouseget = thread(&pauseget_mouse, this);
	p_mouseget.detach();
